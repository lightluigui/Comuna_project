...
def extract_category(filename):
    if "A4" in filename:
        ...
        return "A3"
    elif "Bottons" in filename:
        return "Bottons"
    elif "Adesivos" in filename:
        return "Adesivos"
    else:
...
def organize_files(files):
    for file in files:
        ...
        # Cria a pasta de quantidade
        quantity_folder = input_folder / f'{unit_count} unidades'
...
def extract_unit_count(filename):
    # Supondo que o número de unidades está entre "_" e ".", ex: "arquivo_10.pdf"
    try:
...
#def clean_folders(input_folder, output_folder):
    #for item in Path(input_folder).iterdir():
        #if item.is_dir() and item != Path(output_folder):
            #shutil.rmtree(item)
...
if __name__ == "__main__":
    ...
    # Etapa 3: Separar os arquivos em subpastas por número de unidades
    organize_files(files)

    # Etapa 4: Criar a pasta de output
